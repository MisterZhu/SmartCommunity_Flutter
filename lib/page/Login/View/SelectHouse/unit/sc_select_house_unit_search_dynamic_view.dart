...
/// Description: 搜索框 - 可输入
class SCHouseUnitSearchDynamicView extends StatelessWidget {
  ...
  SCHouseUnitSearchDynamicView({Key? key, this.isShowCancel = false, this.cancelAction, this.valueChangedAction}) : super(key: key);
  ...
  cancelItemClick () {
    ...
    SCSelectHouseUnitSearchStatusController state = Get.find<SCSelectHouseUnitSearchStatusController>();
    SCSelectHouseUnitController sCSelectHouseState = Get.find<SCSelectHouseUnitController>();
  ...
  valueChanged(String value) {
    ...
    SCSelectHouseUnitController state = Get.find<SCSelectHouseUnitController>();
    List<DemoSCHouseUnitModel> houseCommunityList = state.houseCommunityList;
    List<DemoSCHouseUnitModel> searchResultList = [];
    if(value.isNotEmpty) {
      if(houseCommunityList != null){
        for(int i = 0; i < houseCommunityList.length; i++) {
          DemoSCHouseUnitModel demoSCHouseCommunityModel = houseCommunityList[i];
