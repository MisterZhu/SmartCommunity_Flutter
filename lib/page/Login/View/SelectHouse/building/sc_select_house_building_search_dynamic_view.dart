...
/// Description: 搜索框 - 可输入
class SCHouseBuildingSearchDynamicView extends StatelessWidget {
  ...
  SCHouseBuildingSearchDynamicView({Key? key, this.isShowCancel = false, this.cancelAction, this.valueChangedAction}) : super(key: key);
  ...
  Widget body(BuildContext context) {
    if (isShowCancel) {
      ...
      return SCSelectHouseBuildingSearchStaticView();
    }
  }
  ...
  cancelItemClick () {
    ...
    SCSelectHouseBuildingSearchStatusController state = Get.find<SCSelectHouseBuildingSearchStatusController>();
    SCSelectHouseBuildingController sCSelectHouseState = Get.find<SCSelectHouseBuildingController>();
  ...
  valueChanged(String value) {
    ...
    SCSelectHouseBuildingController state = Get.find<SCSelectHouseBuildingController>();
    List<DemoSCHouseBuildingModel> houseCommunityList = state.houseCommunityList;
    List<DemoSCHouseBuildingModel> searchResultList = [];
    if(value.isNotEmpty) {
      if(houseCommunityList != null){
        for(int i = 0; i < houseCommunityList.length; i++) {
          DemoSCHouseBuildingModel demoSCHouseCommunityModel = houseCommunityList[i];
